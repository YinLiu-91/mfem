...
namespace mfem
{
                                 ...
                                 Vector *Vh, Vector *Vk)
   {
      ...
      for (i = 0; i < NumOfElements; i++)
      {
         ...
         kappa = (dim == sdim) ? J.CalcSingularvalue(0) / J.CalcSingularvalue(dim - 1) : -1.0;
         if (Vh)
         {
            (*Vh)(i) = h;
         }
   ...
   void Mesh::AddHexAsTets(const int *vi, int attr)
   {
      static const int hex_to_tet[6][4] =
          {
              {0, 1, 2, 6}, {0, 5, 1, 6}, {0, 4, 5, 6}, {0, 2, 3, 6}, {0, 3, 7, 6}, {0, 7, 4, 6}};
      int ti[4];
      ...
      if (*old_elem_vert == NULL)
      {
         ...
         if (need_elem_vert)
         {
            *old_elem_vert = new Table;
            ...
            for (int i = 0; i < GetNE(); i++)
            {
               (*old_elem_vert)->AddConnections(i, elements[i]->GetVertices(), elements[i]->GetNVertices());
            }
...
#undef VTX
              ...
              int dimension, int space_dimension)
   {
      ...
      int boundary_index_stride = num_boundary_elements > 0 ? Geometry::NumVerts[boundary_type] : 0;
   ...
   Element *Mesh::NewElement(int geom)
   {
      ...
      case Geometry::SEGMENT:
         return (new Segment);
      case Geometry::TRIANGLE:
         return (new Triangle); //Triangle是Element的派生类，以此确定
      case Geometry::SQUARE:
   ...
   Element *Mesh::ReadElementWithoutAttr(std::istream &input)
   {
      ...
      input >> geom;
      el = NewElement(geom); //将代号2，创建三角单元类，此时的基类指针指向一个Triangle
      MFEM_VERIFY(el, "Unsupported element type: " << geom);
      nv = el->GetNVertices(); //这里返回3
      v = el->GetVertices();   //这里v指向Triangle 类中的indices[3]数组
      for (int i = 0; i < nv; i++)
      {
         input >> v[i];
      }
   ...
   Element *Mesh::ReadElement(std::istream &input)
   {
      ...
      input >> attr; //attr属于某一个elements的第一行
      el = ReadElementWithoutAttr(input);
      el->SetAttribute(attr); //设置属性
      ...
      for (int el = 0; el < orig_mesh->GetNE(); el++)
      {
         ...
         for (int j = 0; j < RG.RefGeoms.Size() / nvert; j++)
         {
            ...
            for (int k = 0; k < nvert; k++)
            {
               for (int j = 0; j < spaceDim; ++j)
               {
                  node_coordinates(k * spaceDim + j, NumOfElements) = vertices[v[vertex_map[k]]](j);
               }
            }
            AddElement(elem);
         }
   ...
   void Mesh::EnsureNodes()
   {
      if (Nodes)
      {
         ...
         if (dynamic_cast<const H1_FECollection *>(fec) || dynamic_cast<const L2_FECollection *>(fec))
         {
            return;
         }
   ...
   static inline void Rotate3(int &a, int &b, int &c)
   {
      if (a < b)
      {
         if (a > c)
   ...
   void Mesh::LocalRefinement(const Array<int> &marked_el, int type)
   {
      ...
      if (Dim == 1) // --------------------------------------------------------
      {
         ...
         CoarseFineTr.point_matrices[Geometry::SEGMENT].UseExternalData(seg_children, 1, 2, 3);
                                    ...
                                    VTKFormat format)
   {
      if (format == VTKFormat::ASCII)
      {
         out << static_cast<int>(val) << suffix;
      }
      else
      {
         bin_io::AppendBytes(buf, val);
   ...
   void WriteBinaryOrASCII<float>(std::ostream &out, std::vector<char> &buf,
                                  ...
                                  VTKFormat format)
   {
      if (format == VTKFormat::BINARY)
      {
         bin_io::AppendBytes<double>(buf, val);
      }
      else if (format == VTKFormat::BINARY32)
      {
         bin_io::AppendBytes(buf, val);
      }
      else
      ...
      if (pts_found != npts)
      {
         ...
         for (int k = 0; k < npts; k++)
         {
            ...
            if (ncmesh)
            {
               ...
               {
                  int nn = neigh[e];
                  if (ncmesh->IsGhost(ncmesh->elements[nn]))
                  {
                     continue;
                  }
